...
// Sensor types shown on other pages (excluded from health menu)
$health_exclude_types = ['brocade-poe'];
...
foreach (Sensor::where('device_id', $device['device_id'])->distinct()->pluck('sensor_class') as $sensor_class) {
    // Skip sensor class if all its sensors are excluded types (displayed elsewhere)
    if (! empty($health_exclude_types)) {
        $has_non_excluded = Sensor::where('device_id', $device['device_id'])
            ->where('sensor_class', $sensor_class)
            ->whereNotIn('sensor_type', $health_exclude_types)
            ->exists();
        if (! $has_non_excluded) {
            continue;
        }
